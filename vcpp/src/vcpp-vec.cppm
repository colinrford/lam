...
constexpr vec3 vec(double x, double y, double z) noexcept
{
  return vec3{x, y, z};
}

constexpr vec2 vec(double x, double y) noexcept
{
  return vec2{x, y};
}
...
// Magnitude
constexpr double mag(const vec3& v) noexcept
{
  return std::sqrt(v.x()*v.x() + v.y()*v.y() + v.z()*v.z());
}

constexpr double mag2(const vec3& v) noexcept
{
...
constexpr vec3 norm(const vec3& v) noexcept
{
  return hat(v);
}
...
constexpr vec3 proj(const vec3& a, const vec3& b) noexcept
{
  ...
  if (b_mag2 == 0.0) return vec3{0, 0, 0};
...
constexpr vec3 rotate(const vec3& v, double angle,
                       const vec3& axis = vec3{0, 0, 1}) noexcept
{
  vec3 k = hat(axis);
  ...
  return vec3{
    v.x()*c + k_cross_v.x()*s + k.x()*k_dot_v*(1-c),
...
constexpr double max_component(const vec3& v) noexcept
{
  return std::max({v.x(), v.y(), v.z()});
}

constexpr double min_component(const vec3& v) noexcept
{
