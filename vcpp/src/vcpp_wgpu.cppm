...
inline std::vector<vertex> generate_box()
{
  ...
  for (int f = 0; f < 6; ++f)
  {
    ...
    {
      vertex v{};
      v.position[0] = faces[f][c][0];
      v.position[1] = faces[f][c][1];
      v.position[2] = faces[f][c][2];
      v.normal[0] = n[f][0];
      v.normal[1] = n[f][1];
      v.normal[2] = n[f][2];
      vertices.push_back(v);
...
inline void render_frame_impl(canvas& c)
{
  ...
  auto draw_instances = [&](int mesh_idx, const std::vector<instance_data>& instances, WGPUBuffer& buf,
                            std::size_t& cap) {
    if (instances.empty() || !g_renderer.meshes[mesh_idx].vertex_buffer)
      return;
    ...
    if (inst_size > cap)
    {
      ...
      cap = inst_size * 2;
      buf = create_buffer(static_cast<WGPUBufferUsage>(WGPUBufferUsage_Vertex | WGPUBufferUsage_CopyDst), cap);
    }
    ...
    wgpuRenderPassEncoderSetIndexBuffer(pass, g_renderer.meshes[mesh_idx].index_buffer, WGPUIndexFormat_Uint32, 0,
                                        WGPU_WHOLE_SIZE);
...
inline bool window_should_close_impl() { return g_renderer.window && glfwWindowShouldClose(g_renderer.window); }
...
export inline bool init(canvas& c = scene)
{
  ...
  while (!adapter_data.done)
    wgpuInstanceProcessEvents(g_renderer.instance);
...
export inline void shutdown()
{
  ...
  if (g_renderer.camera_buffer)
    wgpuBufferRelease(g_renderer.camera_buffer);
  ...
  if (g_renderer.bind_group)
    wgpuBindGroupRelease(g_renderer.bind_group);
  if (g_renderer.bind_group_layout)
    wgpuBindGroupLayoutRelease(g_renderer.bind_group_layout);
  if (g_renderer.pipeline)
    wgpuRenderPipelineRelease(g_renderer.pipeline);
  if (g_renderer.depth_view)
    wgpuTextureViewRelease(g_renderer.depth_view);
  if (g_renderer.depth_texture)
    wgpuTextureRelease(g_renderer.depth_texture);
  if (g_renderer.surface)
    wgpuSurfaceRelease(g_renderer.surface);
  if (g_renderer.queue)
    wgpuQueueRelease(g_renderer.queue);
  if (g_renderer.device)
    wgpuDeviceRelease(g_renderer.device);
  if (g_renderer.adapter)
    wgpuAdapterRelease(g_renderer.adapter);
  if (g_renderer.instance)
    wgpuInstanceRelease(g_renderer.instance);
  if (g_renderer.window)
  {
