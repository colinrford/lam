...
// Define field K
using field = lam::cbn::ZqElement<
    std::uint64_t, 18446744069414584321ULL>; // Solinas Prime 2^64 - 2^32 + 1
...
void benchmark(std::size_t N) {
  ...
  std::println("N={:<8} Time={:<10.0f} ns   Norm={:<8.2f} ns/NlogN", N, avg_ns,
               ns_per_n);
}
...
int main() {
  ...
  std::vector<std::size_t> sizes = {4096,   8192,   16384,  32768,  65536,
