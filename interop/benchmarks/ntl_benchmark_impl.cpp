...
bool verify_ntl_multiplication_impl(long N, const long *a_data,
                                    const long *b_data,
  ...
  for (long i = 0; i <= deg; ++i) {
    ...
    if (val != expected_data[i]) {
      std::println(std::cerr, "Mismatch at index {}: NTL={} Local={}", i, val,
                   expected_data[i]);
      return false;
    }
  }
  // Check purely zero beyond degree
  // (Local implementation might have zero padding up to power of 2 size, check
  // strictly logic) We assume expected_data is valid up to 2*N
